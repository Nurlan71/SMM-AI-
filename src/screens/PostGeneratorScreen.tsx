...
const TEMPLATES = [
    { id: '1', text: 'Анонс новой осенней коллекции' },
    { id: '2', text: '5 советов, как выбрать идеальный свитер на зиму' },
    { id: '3', text: 'История нашего бренда: с чего все начиналось' },
];
const HISTORY_KEY = 'smm_ai_post_history';
...
export const PostGeneratorScreen = () => {
    ...
    const handleGenerate = async () => {
        ...
        setLoadingState({ isLoading: true, message: '✍️ Создаем пост... AI подбирает лучшие слова.' });
        setError('');
        setResults([]);
        ...
        try {
            ...
            addToHistory({ id: Date.now().toString(), text: topic });
    ...
    const controls = (
        <>
            ...
            <PromptLibrary templates={TEMPLATES} history={history} onSelect={setTopic} />
            ...
            >
                {loadingState.isLoading ? 'Генерация...' : '✨ Сгенерировать пост'}
            </button>
        </>
    );
    ...
    const resultsPanel = (
        <>
            <h2 style={{fontWeight: 600, width: '100%', textAlign: 'left', marginBottom: '16px'}}>Результаты</h2>
            ...
            {!loadingState.isLoading && results.length === 0 && !error && (
                <EmptyState
                    icon="✍️"
    ...
    return <GeneratorScreenLayout controls={controls} results={resultsPanel} />;
};